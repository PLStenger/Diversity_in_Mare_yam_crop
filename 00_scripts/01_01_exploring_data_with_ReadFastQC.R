...
for(i in id){
  unzip(i, exdir="~/Documents/PostDoc_02_MetaBarcoding_IAC/02_Data/05_Mare_ignames/Diversity_in_Mare_yam_crop/01_quality_check/OUT_PUT_ITS2_fungi")
}
adapter()
...
id <- list.files(pattern = ".zip") # Find all the names of files with html extension
for(i in id){
  unzip(i, exdir="~/Documents/PostDoc_02_MetaBarcoding_IAC/02_Data/05_Mare_ignames/Diversity_in_Mare_yam_crop/01_quality_check/OUT_PUT_V4_bacteria")
}
adapter()
# Run this to obtain all the per base quality of all FastQC in one pdf file ("Per base quality.pdf")
bquality()
